class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3:
                if  ( not(l[2].isdigit()) ):	
                    self._flag = False
                    Parser._error("in line ",n,"Undifend push argument "+l[2])
                    return ""
                else:
    ...
    def _goto(self, lab, n):
        if lab not in self._labelList:
            self._flag = False
            Parser.error("GOTO", n, "Label: " + lab + " does not exist.")
            return ""
        return "@" + self._func + "$" + lab + "\n0;JMP"
    ...
    def _ifgoto(self, lab, n):
        if lab not in self._labelList:
            self._flag = False
            Parser.error("IFGOTO", n, "Label: " + lab + " does not exist.")
            return ""
        return "@SP\nAM=M-1\nD=M+1\n@" + self._func + "$" + lab + "\nD;JEQ"
